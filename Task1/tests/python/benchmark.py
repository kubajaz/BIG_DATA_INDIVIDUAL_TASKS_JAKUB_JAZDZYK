...
def test_multiply_matrices(benchmark):
    ...
    for n in sizes:
        ...
        # Mierzenie użycia pamięci i CPU
        memory_usage = psutil.virtual_memory().used / (1024 ** 2)  # MB
        cpu_usage = psutil.cpu_percent(interval=None)

        results.append({
            "Matrix Size": n,
            "Execution Time (s)": exec_time,  # Użyj .mean dla średniego czasu
            "Memory Usage (MB)": memory_usage,
            "CPU Usage (%)": cpu_usage
        })

        # Poprawiono drukowanie exec_time
...
if __name__ == "__main__":
    pytest.main([__file__, "--benchmark-only"])  # Uruchom benchmark, jeśli plik jest uruchamiany bezpośrednio
